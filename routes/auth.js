...
router.post('/login', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, email, mot_de_passe, role FROM utilisateurs WHERE email = $1',
      [email]
    );
    ...
    res.json({ message: 'Connexion réussie', user: { id: user.id, email: user.email, role: user.role }, token });
  } catch (err) {
